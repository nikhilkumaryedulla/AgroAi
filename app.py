...
def generate_hyperparameter_graphs():
    # Plot optimization history
    optimization_history = optuna.visualization.plot_optimization_history(study)
    optimization_history_html = pio.to_html(optimization_history)

    # Plot parallel coordinate
    parallel_coordinate = optuna.visualization.plot_parallel_coordinate(study)
    parallel_coordinate_html = pio.to_html(parallel_coordinate)

    # Plot slice
    slice_plot = optuna.visualization.plot_slice(study, params=['n_estimators', 'max_depth', 'min_samples_split', 'min_samples_leaf'])
    slice_plot_html = pio.to_html(slice_plot)

    # Plot param importances
    param_importances = optuna.visualization.plot_param_importances(study)
    param_importances_html = pio.to_html(param_importances)

    return optimization_history_html, parallel_coordinate_html, slice_plot_html, param_importances_html


@app.route('/hyperparameter_graph', methods=['POST'])
def hyperparameter_graph():
    if request.method == 'POST':
        optimization_history_html, parallel_coordinate_html, slice_plot_html, param_importances_html = generate_hyperparameter_graphs()

        return render_template('hyperparameter_graph.html',
                               optimization_history_html=optimization_history_html,
                               parallel_coordinate_html=parallel_coordinate_html,
                               slice_plot_html=slice_plot_html,
                               param_importances_html=param_importances_html)
